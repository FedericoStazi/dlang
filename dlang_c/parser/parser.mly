...
%nonassoc UMINUS  /* dummy nonterminal for use with %prec */
...
| INL texpr expr %prec UMINUS        { Past.Inl(get_loc(), $3) }
| INR texpr expr %prec UMINUS        { Past.Inr(get_loc(), $3) }
| FUN LPAREN IDENT COLON texpr RPAREN ARROW expr END
                                     { Past.Lambda(get_loc(), ($3, $8)) }
| LET IDENT COLON texpr EQUAL expr IN expr END
                                     { Past.Let (get_loc(), $2, $6, $8) }
| LET IDENT LPAREN IDENT COLON texpr RPAREN COLON texpr EQUAL expr IN expr END
                                     { Past.LetFun (get_loc(), $2, ($4, $11), $13) }
| CASE expr OF
      INL LPAREN IDENT COLON texpr RPAREN ARROW expr
  BAR INR LPAREN IDENT COLON texpr RPAREN  ARROW expr
  END
                                     { Past.Case (get_loc(), $2, ($6, $11), ($15, $20)) }
// alternative expressions without type expressions
| FUN IDENT ARROW expr END
                                     { Past.Lambda(get_loc(), ($2, $4)) }
| LET IDENT EQUAL expr IN expr END
                                     { Past.Let (get_loc(), $2, $4, $6) }
| LET IDENT IDENT EQUAL expr IN expr END
                                     { Past.LetFun (get_loc(), $2, ($3, $5), $7) }
| INL expr                           { Past.Inl(get_loc(), $2) }
| INR expr                           { Past.Inr(get_loc(), $2) }
| CASE expr OF INL IDENT ARROW expr BAR INR IDENT ARROW expr END
